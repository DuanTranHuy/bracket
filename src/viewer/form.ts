import { Database, helpers } from "../manager";
import {
  StageType,
  RoundRobinMode,
  SeedOrdering,
...
export function stageFormCreator(
  ...
  if (null === parent)
    throw new DOMException(
      "parent with ID: " + configuration.parent_id + " was not found!"
    );
  ...
  stageSelector.onchange = (): void => {
    ...
    switch ((<HTMLInputElement>stageSelector).value) {
      ...
      default:
        throw new DOMException(
          "stage " +
            (<HTMLInputElement>stageSelector).value +
...
export function updateFormCreator(
  ...
  stageSelector.onchange = (): void => {
    ...
    const stage = ((<HTMLInputElement>stageSelector).value ||
      "single_elimination") as StageType;
...
function createMaskFields(
  ...
  switch (stage) {
    ...
    default:
      throw new DOMException(
        `stage ${stage as string} seems to be not implemented yet.`
      );
  }
  ...
  switch (stage) {
    case "round_robin":
      ...
      const roundRobinSettings: StageSettings = {
        seedOrdering: [
          <SeedOrdering>(
            (<HTMLInputElement>(
              document.getElementById(config.html_seed_order_id)
            )).value
          ) ?? "groups.effort_balanced",
        ],
        roundRobinMode:
        ...
        groupCount: parseInt(
          (<HTMLInputElement>document.getElementById(config.html_group_id))
            .value ?? "0"
        ),
      };
      ...
      payload = {
        name:
          (<HTMLInputElement>document.getElementById(config.html_name_id))
            .value ?? "",
      ...
      if (teamNamesValue) payload.seeding = teamNamesValue.split(",");
        ...
        const teamCount = parseInt(
          (<HTMLTextAreaElement>(
            document.getElementById(config.html_team_count_input_id)
          )).value
        );
        payload.seeding = Array.from({ length: teamCount }).map(
          (_, i) => `Team ${i + 1}`
        );
    ...
    case "single_elimination":
      ...
      if (teamNamesValue) payload.seeding = teamNamesValue.split(",");
        ...
        singleEliminationSettings.size =
          helpers.getNearestPowerOfTwo(teamCount);
      }
    ...
    case "double_elimination":
      ...
      const seedOrder = rawSeedOrder.map(
        (order) => order.trim() as SeedOrdering
      );
      ...
      if (teamNamesValue) payload.seeding = teamNamesValue.split(",");
        ...
        doubleEliminationSettings.size =
          helpers.getNearestPowerOfTwo(teamCount);
      }
...
function validateDoubleElimination(config: FormConfiguration): void {
  ...
  if (!grandFinalType || !grandFinalTypes.includes(grandFinalType))
    throw new DOMException(
      "grand_final_type must be one of: " + grandFinalTypes.toString()
    );
...
function validateSingleElimination(config: FormConfiguration): void {
  ...
  const ordering = (<HTMLSelectElement>(
    document.getElementById(config.html_seed_order_id)
  )).value as SeedOrdering;
  if (!ordering || !eliminationOrderings.includes(ordering))
    throw new DOMException(
      "seed_ordering must be one of: " + eliminationOrderings.toString()
    );
}
...
function validateRoundRobin(config: FormConfiguration): void {
  ...
  if (!roundRobinSeeds.includes(ordering))
    throw new DOMException(
      "seed_ordering must be one of " + roundRobinSeeds.toString()
    );
  ...
  if (!roundRobinMode.includes(roundRobinMode))
    throw new DOMException(
      "round_robin_mode must be one of " + roundRobinMode.toString()
    );
}
...
function baseValidation(config: FormConfiguration): void {
  const name = (<HTMLInputElement>document.getElementById(config.html_name_id))
    .value;
  ...
  const stageType = (<HTMLInputElement>(
    document.getElementById(config.html_stage_type_selector_id)
  )).value as StageType;
